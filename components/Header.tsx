...
const Header: React.FC = () => {
  ...
  return (
    <div className="container px-10 mx-auto mb-8">
      <div className="inline-block w-full h-full py-8 border-b border-blue-400">
        ...
        <div className="hidden lg:float-left lg:contents">
          <div className="w-auto pl-[20vw]">
            {
              categories.map((category) => (
                <Link key={category.slug} href={`/category/${category.slug}`}>
                  <span className="ml-4 font-semibold text-white align-middle cursor-pointer md:float-right">
                    {category.name}
                  </span>
                </Link>
              ))
